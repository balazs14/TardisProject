...
datadir = 'datasets/daily_parquets'
...
def align_calls_puts(opt_df):
    ...
    rename_cols = ['ask_amount', 'ask_price', 'bid_amount', 'bid_price', 'symbol',
                   'stale', 'bid_iv','ask_iv', 'mark_iv', 'mark_price','spot_sym']
...
def pcp_breaking_pandas(
    ...
    df['pcpb_fwd_real'] = (df['pcpb_forward'] - cost)
    df['pcpb_bck_real'] = (df['pcpb_backward'] - cost)

    df['pcpb_fwd_real_bp'] = df['pcpb_fwd_real'] / df['capital_fwd'] * 10000
    df['pcpb_bck_real_bp'] = df['pcpb_bck_real'] / df['capital_bck'] * 10000

    df['pcpb_fwd_bp'] = (df['pcpb_forward'] / df['capital_fwd']) * 10000
    df['pcpb_bck_bp'] = (df['pcpb_backward'] / df['capital_bck']) * 10000

    df['pcpb_fwd_ann_bp'] = df['pcpb_fwd_bp'] /  df['tte']
    df['pcpb_bck_ann_bp'] = df['pcpb_bck_bp'] / df['tte']

    df['pcpb_fwd_real_ann_bp'] = df['pcpb_fwd_real_bp'] / df['tte']
    df['pcpb_bck_real_ann_bp'] = df['pcpb_bck_real_bp'] / df['tte']

    df['put_opt_spread_bp'] = (df['put_ask_price_xS'] - df['put_bid_price_xS']) / df['index'] * 10000
    df['call_opt_spread_bp'] = (df['call_ask_price_xS'] - df['call_bid_price_xS']) / df['index'] * 10000

    df['bigger_opt_spread_bp'] = np.maximum(df['put_opt_spread_bp'], df['call_opt_spread_bp'])
    df['smaller_opt_spread_bp'] = np.minimum(df['put_opt_spread_bp'], df['call_opt_spread_bp'])

    df['amu_fwd_bp'] = ( (df['pcpb_forward'] - cost)/(contract_size * df['index']) * 10000 )
    df['amu_bck_bp'] = ( (df['pcpb_backward'] - cost)/(contract_size * df['index']) * 10000 )

    df['contract_size'] = contract_size

    df['fut_mid_price'] = 0.5*df['fut_bid_price'] + 0.5*df['fut_ask_price']
    df['rel_strike'] = df['strike'] / df['fut_mid_price']

    df['mdy'] = df.ts.dt.date
...
def pcp_breaking_polars(
    ...
    ).clip(lower_bound=0) * contract_size).alias('capital_fwd')

    capital_bck = ((
        (index * fut_mgn_rate)
        + (strike * short_put_mgn_rate)
        + (put_bid_price_xS - call_ask_price_xS)
    ).clip(lower_bound=0) * contract_size).alias('capital_bck')

    pcpb_fwd_real = (pcpb_forward - cost).alias('pcpb_fwd_real')
    pcpb_bck_real = (pcpb_backward - cost).alias('pcpb_bck_real')

    pcpb_fwd_real_bp = (pcpb_fwd_real / capital_fwd).alias('pcpb_fwd_real_bp') * 10000
    pcpb_bck_real_bp = (pcpb_bck_real / capital_bck).alias('pcpb_bck_real_bp') * 10000

    pcpb_fwd_bp = ((pcpb_forward / capital_fwd) * 10000).alias('pcpb_fwd_bp')
    pcpb_bck_bp = ((pcpb_backward / capital_bck) * 10000).alias('pcpb_bck_bp')

    pcpb_fwd_ann_bp = (pcpb_fwd_bp / tte).alias('pcpb_fwd_ann_bp')
    pcpb_bck_ann_bp = (pcpb_bck_bp / tte).alias('pcpb_bck_ann_bp')

    pcpb_fwd_real_ann_bp = (pcpb_fwd_real_bp / tte).alias('pcpb_fwd_real_ann_bp')
    pcpb_bck_real_ann_bp = (pcpb_bck_real_bp / tte).alias('pcpb_bck_real_ann_bp')

    put_opt_spread_bp = ((put_ask_price_xS - put_bid_price_xS) / index ).alias('put_opt_spread_bp') * 10000
    call_opt_spread_bp = ((call_ask_price_xS - call_bid_price_xS) /index  ).alias('call_opt_spread_bp') * 10000
    bigger_opt_spread_bp = (pl.max_horizontal(put_opt_spread_bp, call_opt_spread_bp)).alias('bigger_opt_spread_bp')
    smaller_opt_spread_bp = (pl.min_horizontal(put_opt_spread_bp, call_opt_spread_bp)).alias('smaller_opt_spread_bp')

    amu_fwd_bp = ( (pcpb_forward - cost)/(contract_size * index) * 10000).alias('amu_fwd_bp')
    amu_bck_bp = ( (pcpb_backward - cost)/(contract_size * index) * 10000).alias('amu_bck_bp')

    fut_mid_price = (0.5 * fut_bid_price + 0.5 * fut_ask_price).alias('fut_mid_price')
    rel_strike = (strike / fut_mid_price).alias('rel_strike')
    mdy = pl.col('ts').dt.date().alias('mdy')
...
def merge_data(fr,  to=None, freq='1min', output_tag='resampled', force_reload=False, cleanup=False):
    ...
    cols_to_save += ['bid_iv', 'ask_iv', 'mark_iv', 'mark_price']
...
def iter_datasets(fr='2025-01-02', chunksize=100000, cleanup=False):
    ...
    chain_df_iter = read_parquet_chunks(dayinfo.opt_chain_fname, chunksize=chunksize)
    ...
    # using the chain data instead of the options quotes
    df_iters['OPTIONS'] = opt_df_iter
    #df_iters['CHAIN'] = chain_df_iter
...
def download_multiple_csvs(fr='2025-01-02', cleanup=False):
    todo = pl.DataFrame({
        'ex': ['okex-futures', 'okex-options','okex','okex'],# 'okex-options'],
        'dt': ['derivative_ticker', 'quotes', 'quotes','quotes'], #'options_chain'],
        'sym': ['FUTURES', 'OPTIONS','BTC-USDT','ETH-USDT'], #'OPTIONS']
    })
    ...
    #opt_chain_fname = download_single_csv('okex-options', 'options_chain', fr, to, 'OPTIONS', cleanup=cleanup)
    opt_chain_fname = ''
...
def explore(day='2026-01-01', force_reload=False, freq='5min', output_tag='test'):
    # read the prepared data
    resampled_df = merge_data(day, freq=freq, output_tag=output_tag,
                              force_reload=force_reload, cleanup=False)
...
def main(fr='2026-01-01', to=None, cleanup=False, output_tag='test', force_reload=False, freq='1min'):
    if to is None:
        to = fr
    pcpbs = []
    for day in pd.date_range(fr, to, freq='1D'):
        ...
        #pcpb.write_parquet(f'{datadir}/okex_pcpb_{output_tag}_{day.date()}.parquet')
...
if __name__ == '__main__':
    ...
    pcpb_df = main(day, to, cleanup=True, output_tag='prod5', freq='5min')
