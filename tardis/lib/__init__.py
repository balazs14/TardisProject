...
def config_logging(fname):
    ...
    formatter = SafeFormatter("%(simts)s | %(levelname)-8s | %(name)-20s | %(funcName)-20s: %(message)s")
    
    handler.setFormatter(formatter)
    pkg_logger.addHandler(handler)


def package_set_log_level(level=logging.INFO, file_level=logging.INFO):
    pkg_logger = logging.getLogger(__name__) # root logger for package
    #pkg_logger.setLevel(logging.DEBUG) # logger's level is applied first, then handlers filter
    level_int = level if isinstance(level, int) else logging._nameToLevel[level]
    file_level_int = file_level if isinstance(file_level, int) else logging._nameToLevel[file_level]
    most_verbose_level = min(level_int, file_level_int)
    pkg_logger.setLevel(most_verbose_level) # logger's level is applied first, then handlers filter
    for h in pkg_logger.handlers:
