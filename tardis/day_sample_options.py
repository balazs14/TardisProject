...
def _normalize_trades(exchange, df: pl.DataFrame, timestamp_unit: str) -> pl.DataFrame:
    ...
    logger.debug("_normalize_trades start rows=%d columns=%s", df.height, df.columns)
...
def _normalize_okex_quotes(df: pl.DataFrame, timestamp_unit: str) -> pl.DataFrame:
    ...
    logger.debug("_normalize_okex_quotes start rows=%d columns=%s", df.height, df.columns)
    df = _to_ts(df, "timestamp", "ts", timestamp_unit)
    parts = pl.col("symbol").str.split("-")

    df = df.with_columns([parts.list.get(0).alias("_A"), parts.list.get(1).alias("_B"), parts.list.get(2, null_on_oob=True).alias("exp_str"), parts.list.get(3, null_on_oob=True).alias("_D"), parts.list.get(4, null_on_oob=True).alias("_E")])
    logger.debug("_normalize_okex_quotes parsed symbol parts rows=%d", df.height)
    df = df.with_columns([
        pl.when(pl.col("_D").is_not_null()).then(pl.col("_D").cast(pl.Float64, strict=False)).otherwise(pl.lit(None, dtype=pl.Float64)).alias("strike"),
        pl.when(pl.col("exp_str").is_null()).then(pl.lit("S")).when(pl.col("_E").is_null()).then(pl.lit("F")).otherwise(pl.col("_E")).alias("pc"),
        pl.when((pl.col("_B").is_in(["USD", "USDT"]) & pl.col("_E").is_in(["C", "P"]))).then(pl.lit("USDT")).otherwise(pl.col("_B")).alias("_BT"),
    ...
    logger.debug("_normalize_okex_quotes derived fields rows=%d columns=%s", df.height, df.columns)
...
def _normalize_deribit_quotes(df: pl.DataFrame, timestamp_unit: str) -> pl.DataFrame:
    ...
    df = df.with_columns([
        parts.list.get(0).alias("_A"), parts.list.get(1, null_on_oob=True).alias("_B"),
        parts.list.get(2, null_on_oob=True).alias("_C"), parts.list.get(3, null_on_oob=True).alias("_D")
    ])
    ...
    df = df.with_columns([pl.col("_C").cast(pl.Float64, strict=False).alias("strike")])
    ...
    df = df.with_columns([
        ...
        .otherwise(pl.lit(None, dtype=pl.Datetime(timestamp_unit))).alias("exp"),
        pl.when(pl.col("_B").is_not_null() & (pl.col("_B") != "PERPETUAL"))
        .then(pl.col("_B").str.to_uppercase())
        .otherwise(pl.lit(None, dtype=pl.String)).alias("exp_str")
    ])
    ...
    df = df.with_columns([
        ...
        .otherwise(pl.col("symbol")).alias("fut_sym"),
        pl.when(pl.col("_A").is_in(["BTC", "ETH"]))
        .then(pl.concat_str([pl.col("_A"), pl.lit("USDC")], separator="_"))
        .otherwise(pl.col("_A")).alias("spot_sym"),
        pl.when(pl.col("_A").is_in(["BTC", "ETH"]))
        .then(pl.concat_str([pl.col("_A"), pl.lit("USD")], separator=""))
        .otherwise(pl.col("_A")).alias("ref_sym")
    ])
    cols = ["ts", "symbol", "ref_sym", "fut_sym", "spot_sym", "exp", "exp_str", "pc", "strike", 
            "bid_price", "ask_price", "bid_amount", "ask_amount", "bid_iv", "ask_iv", "mark_iv", 
             "mark_price", "stale"]
    missing = [c for c in cols if c not in df.columns]
    if missing:
        df = df.with_columns([pl.lit(False if c == "stale" else None).alias(c) for c in missing])
...
def _resample_normalized_quotes(df: pl.DataFrame, freq: str, timestamp_unit: str) -> pl.DataFrame:
    ...
    base_value_cols = ["ref_sym", "fut_sym", "spot_sym", "exp", "exp_str", "pc", "strike", 
                       "bid_price", "ask_price", "bid_amount", "ask_amount", "bid_iv", 
                       "ask_iv", "mark_iv", "mark_price"]
...
def _align_calls_puts_with_legs(resampled_df: pl.DataFrame, resampled_trade_df: pl.DataFrame) -> pl.DataFrame:
    ...
    # --- partition legs ---
    fut = resampled_df.filter(pl.col("pc") == "F").rename({"ask_amount": "fut_ask_amount", "ask_price": "fut_ask_price", "bid_amount": "fut_bid_amount", "bid_price": "fut_bid_price", "stale": "fut_stale", "exp": "fut_exp"})
    spot = resampled_df.filter(pl.col("pc") == "S").rename({"ask_amount": "spot_ask_amount", "ask_price": "spot_ask_price", "bid_amount": "spot_bid_amount", "bid_price": "spot_bid_price"})
...
def testme():
    return day_sample('2026-01-01','okex',SampleConfig(freq='5min'))

def day_sample(day: str, exchange: str, config: SampleConfig = SampleConfig()) -> pl.DataFrame:
    ...
    if trade_frames:
        ...
        normalized_trades = pl.DataFrame([])
    logger.debug(
